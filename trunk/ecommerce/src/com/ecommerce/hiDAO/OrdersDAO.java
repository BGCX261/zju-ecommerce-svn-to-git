 ...
 * 
 * @see com.ecommerce.hiDAO.Orders
...
public class OrdersDAO extends BaseHibernateDAO {
	...
	public Orders findById(java.lang.Integer id) {
		...
		try {
			Orders instance = (Orders) getSession().get(
					"com.ecommerce.hiBean.Orders", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Orders instance) {
		...
		try {
			List results = getSession().createCriteria(
					"com.ecommerce.hiBean.Orders").add(Example.create(instance))
					.list();
	...
	public List findByUserName(String userId) {
		log.debug("finding all Orders instances");
		try {
	...
	public Orders findByTimeStamp(Timestamp t) {
		log.debug("finding all Orders instances");
		try {
			String queryString = "from Orders where submintTime = '" + t+"'";
			Query queryObject = getSession().createQuery(queryString);
			return (Orders)queryObject.list().get(0);
		} catch (RuntimeException re) {
			log.error("find all failed", re);
			throw re;
		}
